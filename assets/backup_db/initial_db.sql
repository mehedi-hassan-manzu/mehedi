...
CREATE TABLE IF NOT EXISTS `fb_msg_manager` (
  ...
  `social_media` enum('ig','fb') NOT NULL DEFAULT 'fb',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `messenger_bot` (
  ...
  `keyword_type` enum('reply','post-back','no match','get-started','email-quick-reply','phone-quick-reply','location-quick-reply','birthday-quick-reply','story-mention') COLLATE utf8mb4_unicode_ci NOT NULL DEFAULT 'reply',
...
INSERT INTO `menu` (`name`, `icon`, `url`, `serial`, `module_access`, `have_child`, `only_admin`, `only_member`, `add_ons_id`, `is_external`, `header_text`, `is_menu_manager`, `custom_page_id`) VALUES('Visual Flow Builder', 'fas fa-project-diagram', 'visual_flow_builder/flowbuilder_manager', (SELECT serial FROM menu as menu2 WHERE url='messenger_bot'), '315', '0', '0', '0', 0, '0', '', '0', 0);
INSERT INTO `menu` (`id`, `name`, `icon`, `url`, `serial`, `module_access`, `have_child`, `only_admin`, `only_member`, `add_ons_id`, `is_external`, `header_text`, `is_menu_manager`, `custom_page_id`) VALUES(NULL, 'Comment Growth Tools', 'fas fa-layer-group', 'comment_automation/comment_growth_tools', (SELECT serial FROM menu as menu2 WHERE only_admin='1' ORDER BY `serial` DESC LIMIT 1), '80,201,202,204,206,220,222,223,251,256,278,279', '0', '0', '0', 0, '0', 'Comment Feature', '0', 0);
INSERT INTO `menu` (`name`, `icon`, `url`, `serial`, `module_access`, `have_child`, `only_admin`, `only_member`, `add_ons_id`, `is_external`, `header_text`, `is_menu_manager`, `custom_page_id`) VALUES ( 'Live Chat', 'fas fa-comment-alt', 'subscriber_manager/livechat', (select serial from menu as menu2 where url='subscriber_manager'), '', '0', '0', '0', 0, '0', '', '0', 0);
...
INSERT INTO `menu_child_1` (`name`, `url`, `serial`, `icon`, `module_access`, `parent_id`, `have_child`, `only_admin`, `only_member`, `is_external`, `is_menu_manager`, `custom_page_id`) VALUES('Check Update v2', 'update_system/update_list_v2', (SELECT serial FROM menu_child_1 as menu_child_copy WHERE url='update_system/index'), 'fas fa-leaf', '', (SELECT parent_id FROM menu_child_1 as menu_child_copy WHERE url='update_system/index'), '0', '1', '0', '0', '0', '0');
...
ALTER TABLE `instagram_reply_template` ADD `page_id` INT(11) NOT NULL AFTER `private_message_offensive_words`;

ALTER TABLE `instagram_reply_template` ADD `ig_username` VARCHAR(200) NOT NULL AFTER `page_id`;

ALTER TABLE `ecommerce_product` ADD `related_product_ids` VARCHAR(255) NOT NULL AFTER `woocommerce_price_html`;

ALTER TABLE `ecommerce_product` ADD `upsell_product_id` INT(11) NOT NULL AFTER `related_product_ids`;

ALTER TABLE `ecommerce_product` ADD `downsell_product_id` INT(11) NOT NULL AFTER `upsell_product_id`;

ALTER TABLE `ecommerce_product` ADD `is_featured` ENUM('0','1') NOT NULL DEFAULT '0' AFTER `downsell_product_id`;